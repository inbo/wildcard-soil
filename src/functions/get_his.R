...
get_his <- function(include_extra_plots = TRUE,
  ...
  # There are two possible sources for the HIS sites
  # 1. "metadata_table": General Google Drive link (which can still be updated)
  # 2. "his_ids" (default): the snapshot from summer 2025 (208 plots).
  #    Advantage of this version is the fact that it cannot be updated anymore
  #    (since harmonised composed_site_id in the app data identification file
  #     and sample lists are based on this version of summer 2025)
  #    Note that this "snapshot" does not contain the finally submitted plot_ids
  #    by the partners (submitted with app)
  ...
  if (source == "his_ids") {
    ...
    his <- read_excel("./data/additional_data/his_ids.xlsx") %>%
      select(-id_sample_pretreatment) # not needed here

    # Update plot_ids based on data from the app if wanted
    ...
    if (add_plot_ids_from_app == TRUE) {
      ...
      his <- his %>%
        mutate(
          # Make a separate column with plot_id from Bialowieza since these
          # should get priority anyhow
          plot_id_bial = case_when(
            grepl("WULS__1__", composed_site_id) ~ plot_id,
            TRUE ~ NA_character_),
          plot_id = coalesce(plot_id_bial, plot_id_harmonized, plot_id),
          plot_code = paste(
            institute,
            res_id_inst,
            sub_id,
            plot_id,
            sep = "__")) %>%
        select(-plot_id_harmonized, -plot_id_bial) %>%
  ...
  } # End of "if source == metadata_table"
